...
function init_job_dir(dir)
	if string.sub(dir, #dir, #dir) == "/" then
		print("Slave directory must not end with a /")
		return false
	end
...
if not production and arg then
	...
	if arg[4] then splayd.settings.job.network.end_port = tonumber(arg[5]) end
end
...
-- uptime: 16:10  up 5 days, 31 mins, 8 users, load averages: 0.14 0.18 0.16
-- TODO *BSDs
...
if splayd.settings.controller.ip ~= "127.0.0.1" and
	...
	-- 2) if the controller wants to blacklist its IPs, it can using the blacklist
	--	mechanism
