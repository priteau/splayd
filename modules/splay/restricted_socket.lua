...
local function tcp_sock_wrapper(sock)
	...
	if sock.close then
		new_sock.close = function(self)
			...
			if not sock:getsockname() then
				...
				--l_o:debug("Total TCP sockets: "..total_tcp_sockets)
				sock:close()
			end
		end
	end
	...
	-- recursivly (not directly in tcp_sock_wrapper() because we
	...
	if sock.accept then
		new_sock.accept = function(self)
			...
			-- socket is too high, we will close the socket immediatly.
			local s, err = sock:accept()
			if not s then return nil, err end

			if total_tcp_sockets >= max_sockets then
				s:close()
				l_o:warn("Accept restricted, too much sockets: "..total_tcp_sockets)
				return nil, "restricted"
			end
...
-- Create a sandbox array arround a true udp socket. The "base" socket is only
-- an array.
...
function wrap(sock)
	...
	new_sock.tcp = function()
		...
		if total_tcp_sockets + total_udp_sockets >= max_sockets then
			return nil, "restricted"
		end
